...
folderdirect<-c("LA1589","SA1","SA2","SA29","SA32")
realfinal<-c()
for (fd in folderdirect) {
  setwd(fd)
  ...
  for (i in 1:length(corrs)) {
    ...
    onecorr <-cbind("corrthresh" = rep(corrsnumbs[i]), "Expr_Data_Origin" = rep(fd),
                    onecorr[,1:7],"sv_size_(bp)" = onecorr$V3-onecorr$V2,onecorr[,8:9])
    final<-rbind(onecorr,final)
    setwd("../")
  }
  realfinal<-rbind(realfinal,final)
  setwd("../")
}